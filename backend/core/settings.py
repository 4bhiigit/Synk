...
SECRET_KEY = os.environ.get('SECRET_KEY', 'django-insecure-chat-app-secret-key-change-in-prod-!89x#^90')
...
# Database
# Uses DATABASE_URL (PostgreSQL on Supabase/Neon/Render) if defined, else SQLite for local development
...
# Django REST Framework Configuration with Throttling
REST_FRAMEWORK = {
...
CORS_ALLOWED_ORIGINS = [
    ...
    for origin in os.environ.get(
        'CORS_ALLOWED_ORIGINS',
        'http://localhost:5173,http://127.0.0.1:5173,http://localhost:3000,http://127.0.0.1:3000,capacitor://localhost,http://localhost'
    ).split(',')
    if origin.strip()
]
