...
export const ChatArea = ({ activeRoom, onBack, onMessageSent }) => {
  ...
  return (
    <div
      className={`flex-1 flex flex-col h-full max-h-full min-h-0 bg-black relative overflow-hidden transition-colors duration-500 ${
        sentimentTone === 'joy'
    ...
    >
      ...
      {isSelectMode ? (
        ...
        /* Clean Interactive Chat Header */
        <div className="px-3.5 py-3 md:px-4 md:py-3.5 bg-[#0d0d10] border-b border-white/10 flex items-center justify-between z-10 select-none flex-shrink-0 shadow-sm">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1 mr-2">
            <button
              onClick={onBack}
              className="md:hidden btn-icon p-2 text-white hover:bg-white/10 active:scale-95 transition-all flex-shrink-0"
              title="Back to Chats"
            >
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-bold text-white text-sm leading-tight flex items-center gap-1.5 truncate">
                <span className="truncate">{displayName}</span>
                {isLocked && <Lock className="w-3 h-3 text-amber-400 flex-shrink-0" title="Vault Protected" />}
                {disappearingTimer > 0 && (
                  <span className="px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[10px] font-mono font-semibold flex items-center gap-0.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
            ...
            {!isChannel && (
              <button
                ...
                className="btn-icon p-2 sm:p-2.5 hover:text-emerald-400 hover:bg-emerald-500/10 active:scale-95 transition-all"
            ...
            {!isChannel && (
              <button
                ...
                className="btn-icon p-2 sm:p-2.5 hover:text-indigo-400 hover:bg-indigo-500/10 active:scale-95 transition-all"
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-3 relative select-text overscroll-contain"
      >
        {loadingHistory ? (
